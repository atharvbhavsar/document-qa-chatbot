...
import { CHUNK_SIZE, CHUNK_OVERLAP, EMBEDDING_MODEL, genAI, USE_OLLAMA } from './config';
...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  try {
    if (!genAI) {
      throw new Error('Google API is not configured. Please set USE_OLLAMA=false and provide a valid GOOGLE_API_KEY');
    }
